...
c.on('ready', () => {
    ...
    c.exec(`docker ps --filter name=${apiContainerName} --format "{{.ID}}"`, (err, stream) => {
        ...
        stream.on('close', () => {
            ...
            const cmd = `docker exec ${containerId} node -e "const fs=require('fs'); fs.mkdirSync('/app/scripts', {recursive:true}); fs.writeFileSync('/app/scripts/migrate-motos.js', Buffer.from('${b64}', 'base64')); console.log('File written');"`;

            c.exec(cmd, (err, stream) => {
                stream.on('close', () => {
                    console.log('Running script...');
                    c.exec(`docker exec ${containerId} node scripts/migrate-motos.js`, (err, stream) => {
